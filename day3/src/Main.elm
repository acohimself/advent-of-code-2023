...
parseEngine string =
    string
        |> String.lines
        |> List.indexedMap (\y l -> List.indexedMap (\x c -> ( ( x, y ), c )) (String.toList l))
        |> List.concat
        |> Dict.fromList


isSymbol : Position -> Char -> Bool
isSymbol p c =
...
filterDigits engine ( x, y ) =
    case Dict.get ( x, y ) engine of
        Nothing ->
            False
...
findLeftMostDigitInNumber engine ( x, y ) =
    if x == 0 then
        ( x, y )

    else
        case Dict.get ( x - 1, y ) engine of
            Nothing ->
                ( x, y )

            Just c ->
                if Char.isDigit c then
                    findLeftMostDigitInNumber engine ( x - 1, y )

                else
                    ( x, y )


getStartPositionOfNumbers : Engine -> List Position -> List Position
getStartPositionOfNumbers engine positions =
    positions
        |> List.filter (filterDigits engine)
        |> List.map (findLeftMostDigitInNumber engine)
        |> unique


getValueFromPosition : Engine -> Position -> List Char
getValueFromPosition engine ( x, y ) =
    case Dict.get ( x, y ) engine of
        Nothing ->
            []
        ...
        Just c ->
            case Char.isDigit c of
                ...
                True ->
                    c :: getValueFromPosition engine ( x + 1, y )
...
part1 input =
    ...
    in
    engine
        |> Dict.filter isSymbol
        |> Dict.keys
        |> List.map neighbors
        |> List.concat
        |> getStartPositionOfNumbers engine
        |> List.map (getValueFromPosition engine)
...
part2 input =
    ...
    in
    engine
        |> Dict.filter isAsterisk
        |> Dict.keys
        |> List.map neighbors
        |> List.map (getStartPositionOfNumbers engine)
        |> List.filter (\l -> List.length l == 2)
        |> List.map (\l -> List.map (getValueFromPosition engine) l)
        |> List.map (List.map (\l -> String.fromList l))
        |> List.map (\l -> List.filterMap String.toInt l)
    ...
    div []
        [ div [ class "container" ]
            [ div [ class "w-1/2 mx-auto" ]
                [ h2 [ class "text-xl" ] [ text "Day 3: Gear Ratios" ]
                ]
